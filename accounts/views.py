...
def login_view(request):
    if request.method == "POST":
        ...
        if user is not None and user.is_active == False:  # noqa: E712
            messages.error(request, "User is not yet activated by admin")
            return render(request, "accounts/login.html")
        elif user is not None and user.is_active == True:  # noqa: E712
            login(request, user)
...
def activateEmail(request, user):
    ...
    email = EmailMessage(
        mail_subject, message, to=ast.literal_eval(settings.ADMINISTRATEURS_EMAILS)
    )
    if email.send():
...
def activate(request, uidb64, token):
    ...
    if user is not None and account_activation_token.check_token(user, token):
        ...
        message = render_to_string(
            "accounts/confirmation_activate_account_email.html",
            {
                "user": user.username,
            },
        )
