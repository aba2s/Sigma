...
def validate_name(value):
    if len(value) < 3:
        raise ValidationError("{} is too sort for an advertiser name".format(value))


def uploaded_files_directory(instance, filename):
    return "files/{}/{}_{}".format(instance.dsp, filename, datetime.datetime)
...
class CampaignNamingTool(models.Model):
    ...
    format_choices = (
        ...
        ("Impact.", "Impact."),
        ("Interst.", "Interst."),
        ("Interst. video", "Interst. video"),
        ("Native", "Native"),
        ("Parallax", "Parallax"),
        ("Vid. pre-roll", "Vid. pre-roll"),
        ("Vid. in-read", "Vid. in-read"),
        ("Vid. in-banner", "Vid. in-banner"),
        ("Vid. in-banner", " Social + IAB"),
        ("Vid. in-banner", "Social"),
    )
    ...
    device = models.CharField(
        max_length=30, choices=device_choices, default="Multi-device"
    )
    type_of_format = models.CharField(
        max_length=30, choices=format_choices, default="IAB"
    )
    ...
    def __str__(self):
        ...
        return "{}-{} - {} - {} - {} - {} - {}".format(
            self.year.strip(),
            month.strip(),
            self.advertiser.strip(),
            self.name.strip(),
            self.device.strip(),
            self.type_of_format.strip(),
            self.kpi.strip(),
        )
...
class UserInsertionOrderByDsp(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    insertion_order = models.ForeignKey(UserInsertionOrder, on_delete=models.CASCADE)
...
class InsertionOrdersCommonFields(models.Model):
    ...
    insertion_order = models.ForeignKey(
        UserInsertionOrder, on_delete=models.CASCADE, db_column="insertion_order"
    )
...
class AsynchroneTask(models.Model):
    ...
    batch_name = models.ForeignKey(
        BatchName, db_column="dsp", on_delete=models.CASCADE, null=True
    ) 
    ...
    def __str__(self):
        return self.batch_name + "->" + self.task
...
class File(models.Model):
    ...
    task = models.ForeignKey(AsynchroneTask, on_delete=models.CASCADE)
    name = models.CharField(max_length=240, help_text="Name of the file")
    # Location of the file on the server.
    file_location = models.FileField(
        verbose_name="File", upload_to=uploaded_files_directory
    )
